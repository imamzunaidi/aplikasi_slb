...
if (isset($_POST['insert'])) {
    ...
    $query_nik = "SELECT * FROM tbl_murid where nik ='$nik'";
    $cek_nik = mysqli_query($conn, $query_nik);

    $data_nik = mysqli_num_rows($cek_nik);

    $query_username = "SELECT * FROM tbl_wali_murid where username ='$username'";
    $cek_username = mysqli_query($conn, $query_username);

    $data_username = mysqli_num_rows($cek_username);


    if($data_nik > 0){
        $_SESSION['message'] = 'Nik tidak boleh sama !!';
        $_SESSION['message_type'] = 'danger';
        header('Location: '.$base_url.'walimurid/pendaftaran.php');
    }else if($data_username > 0){
        $_SESSION['message'] = 'Username tidak boleh sama !!';
        $_SESSION['message_type'] = 'danger';
        header('Location: '.$base_url.'walimurid/pendaftaran.php');
    }else{
